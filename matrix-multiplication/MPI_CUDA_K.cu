...
{
  if (code != cudaSuccess) {
    cerr << "cuda_assert: " << cudaGetErrorString(code) << " " << file << ":" << line;
    exit(code);
  }
}

__global__ void multiply_matrices_k(
    const double* dev_matrix_a,
    const double* dev_matrix_b,
    int m_a,
    int n_a,
    int n_b,
    double* dev_matrix_c)
{
  ...
  const uint m = (blockIdx.x * blockDim.x) + threadIdx.x;
  const uint n = (blockIdx.y * blockDim.y) + threadIdx.y;
    ...
    double* matrix_c)
{
  ...
  const size_t sz_matrix_a = m_a * n_a * sizeof(double);
  double* dev_matrix_a;
  CUDA_CHECK( cudaMalloc(&dev_matrix_a, sz_matrix_a) );

  // allocate GPU memory for matrix B
  const size_t sz_matrix_b = n_a * n_b * sizeof(double);
  double* dev_matrix_b;
  CUDA_CHECK( cudaMalloc(&dev_matrix_b, sz_matrix_b) );

  // allocate GPU memory for output array
  const size_t sz_matrix_c = m_a * n_b * sizeof(double);
  double* dev_matrix_c;
  CUDA_CHECK( cudaMalloc(&dev_matrix_c, sz_matrix_c) );

  // Copy input arrays into GPU memory
  CUDA_CHECK( cudaMemcpy(dev_matrix_a, matrix_a, sz_matrix_a, cudaMemcpyHostToDevice) );
  CUDA_CHECK( cudaMemcpy(dev_matrix_b, matrix_b, sz_matrix_b, cudaMemcpyHostToDevice) );
  ...
  // do the work
  multiply_matrices_k<<<grid_dim, block_dim>>>(dev_matrix_a, dev_matrix_b, m_a, n_a, n_b, dev_matrix_c);

  // copy result back into host memory
  CUDA_CHECK( cudaMemcpy(matrix_c, dev_matrix_c, sz_matrix_c, cudaMemcpyDeviceToHost) );

  CUDA_CHECK( cudaFree(dev_matrix_a) );
  CUDA_CHECK( cudaFree(dev_matrix_b) );
  CUDA_CHECK( cudaFree(dev_matrix_c) );
}
