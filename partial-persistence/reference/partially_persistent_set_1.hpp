#ifndef __PARTIALLY_PERSISTENT_BINARY_TREE_1_HPP
#define __PARTIALLY_PERSISTENT_BINARY_TREE_1_HPP
...
class partially_persistent_set_1
{
    struct node_t;
    ...
    std::optional<_Key> find(unsigned int queryVersion, const _Key &value) const
    {
        ...
        while (node) {
            if (_Compare()(value, node->value)) {
                if (node->mod && queryVersion >= node->mod->version && node->mod->left) {
                    node = node->mod->ptr;
                } else {
    ...
    std::pair<std::optional<_Key>, bool> insert(const _Key &value)
    {
        ...
        while (current_node) {
            ...
            if (_Compare()(value, current_node->value)) {
                if (current_node->mod && current_node->mod->left) {
                    current_node = current_node->mod->ptr;
                } else {
        ...
        while (referencing_node) {
            ...
            if (_Compare()(current_node->value, referencing_node->value)) {
                if (referencing_node->mod) {
                    ...
                    new_node->parent = referencing_node->parent;
                    new_node->right_child = referencing_node->mod->left ? referencing_node->right_child : referencing_node->mod->ptr;
                    new_node->value = referencing_node->value;
                } else {
                    referencing_node->mod.reset(new mod_t());
                    referencing_node->mod->left = true;
                    referencing_node->mod->ptr = current_node;
                ...
                if (referencing_node->mod) {
                    new_node.reset(new node_t());
                    new_node->left_child = referencing_node->mod->left ? referencing_node->mod->ptr : referencing_node->left_child;
                    new_node->parent = referencing_node->parent;
                    new_node->right_child = current_node;
                    new_node->value = referencing_node->value;
                } else {
                    referencing_node->mod.reset(new mod_t());
                    referencing_node->mod->left = false;
                    referencing_node->mod->ptr = current_node;
                    referencing_node->mod->version = version;
                    break;
                }
